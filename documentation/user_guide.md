...
One caveat is worth stating plainly, because it decides whether the number you report is the number you meant. Pairing by index compares the n-th lowest spin-up band with the n-th lowest spin-down band, which is the physical splitting only while the two channels remain in the same band order. Once a splitting grows comparable to the spacing between neighboring bands, those two are no longer the same band and the pairing compares unrelated states, giving a value that is too small. Band folding shows this at its starkest: on a `supercell(2)` of the square altermagnet the folded bands at each k come from four primitive k-points at once, index pairing mixes them, and the reported maximum drops from `4*am` to `2*am` even though the spectrum is unchanged. So use the natural magnetic unit cell, and in dense multiorbital cells -- where the level spacing is small -- treat the result as a lower bound on the true maximum rather than the maximum itself.
