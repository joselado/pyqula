  ...
  !!!! AB linear response !!!!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine calculate_xychi(wf_chi_i,ene_chi_i,&
                           wf_chi_j,ene_chi_j, &
                           ene_chi, &
                           temperature_chi, &
                           delta, &
  ...
  complex (kind=8),intent(out) :: chi_total(num_ene_chi,norbitals*norbitals/4)  ! WF i
...
! loop over the different matrices of linear response
        kab = 1
        do jab=1,norbitals/2   
        do iab=1,norbitals/2   
    ! calculate only at zero temperature
        ! calculate the matrix elementes <A><B>
          vawwbv = conjg(wftmp1(2*iab-1))*wftmp2(2*iab) ! first matrix element
          vawwbv = vawwbv * conjg(wftmp2(2*jab))*wftmp1(2*jab-1) ! second
