"""3D example: a two-orbital CsCl-like cubic lattice.

Same recipe as the 2D square-lattice example, just with an extra periodic
direction -- hard-coded Bloch Hamiltonian, 2 orbitals per unit cell (A at
the cell corner, B at the cell centre) with on-site energies eps1/eps2,
same-orbital nearest-neighbour hopping t1/t2 along x, y and z, and a
constant inter-orbital hybridization t12 (periodic-gauge convention -- see
``_tb_utils.py``'s module docstring)::

    H(k) = [[eps1 + 2*t1*(cos(2*pi*kx)+cos(2*pi*ky)+cos(2*pi*kz)),  t12                                                  ],
            [t12,                                                  eps2 + 2*t2*(cos(2*pi*kx)+cos(2*pi*ky)+cos(2*pi*kz))]]

2 orbitals -> 2 Wannier functions -- since nothing is truncated
...
def hamiltonian_k(k_frac: np.ndarray) -> np.ndarray:
    kx, ky, kz = k_frac
    cos_sum = np.cos(2 * np.pi * kx) + np.cos(2 * np.pi * ky) + np.cos(2 * np.pi * kz)
    return np.array([
...
def main():
    mp_grid = np.array([4, 4, 4], dtype=np.int32)
    a = 3.0  # cubic lattice constant (Ang)
    real_lattice = a * np.eye(3)
    kpt_latt = monkhorst_pack(mp_grid)

    orbital_positions_frac = np.array([[0.0, 0.0, 0.0], [0.5, 0.5, 0.5]])  # sites A (corner), B (centre)
    atoms_cart = orbital_positions_frac @ real_lattice.T

    setup_result = wannier90.setup(
        "cubic_lattice", mp_grid, kpt_latt, real_lattice, NUM_ORBITALS,
        ["A", "B"], atoms_cart,
    ...
    M_matrix, A_matrix, eigenvalues = build_overlaps(
        ...
        trial_widths=[0.25, 0.9], periodic_dims=[0, 1, 2],  # deliberately different widths -- see _tb_utils
    )
    ...
    run_result = wannier90.run(
        "cubic_lattice", setup_result, mp_grid, kpt_latt, real_lattice, ["A", "B"], atoms_cart,
        M_matrix, A_matrix, eigenvalues, backend="python",
    )
    report(run_result, "3D cubic lattice", initial_omega=initial_spread(A_matrix, M_matrix, setup_result.nnlist))
