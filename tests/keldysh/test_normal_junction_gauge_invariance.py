...
@pytest.mark.parametrize("central", [
    lambda h: {"central": [h.copy()]},          # single dense central site
    lambda h: {"central": [h.copy(), h.copy()]},  # block-diagonal central sites
])
def test_explicit_central_region_is_rejected(central):
    """get_dc_current only supports heterostructures with no explicit
    central region (heterostructures.build(h1,h2), the two leads directly
    weak-linked). Testing found a confirmed, unresolved systematic error
    for junctions with an explicit central Hamiltonian (see the Floquet-
    Keldysh section of the user guide), so this case must raise a clear
    NotImplementedError rather than silently return an inaccurate number."""
    h0 = geometry.chain().get_hamiltonian()
    h1 = h0.copy()
    h1.turn_nambu()
    h2 = h1.copy()

    HT = heterostructures.build(h1.copy(), h2.copy(), **central(h1))
    HT.set_coupling(0.6)
    HT.delta = 1e-4
    with pytest.raises(NotImplementedError):
        HT.get_dc_current(0.3, nmax=6, nmax_max=10, tol=1e-3)
