...
def test_island_long_range_scf_magnetization_matches_reference(tmp_path, monkeypatch):
    ...
    from a known-good run. Re-recorded when get_magnetization's default
    changed from the mean-field exchange field to the moment it induces. Marked slow: SCF convergence itself (not the
    ...
    assert np.isclose(np.sum(m), -2.9999999999999942, atol=1e-4)
